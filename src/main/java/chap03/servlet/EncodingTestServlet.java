 ...
 */
@WebServlet(
	description = "한글 인코딩을 테스트하는 서블릿입니다", 
	urlPatterns = { "/encodingTest" })
public class EncodingTestServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;  
	
	/*
		GET방식 요청은 URL뒤에 데이터가 추가되어 도착한다.
		URL의 해석은 Tomcat이 담당하기 때문에 URL뒤에 추가된 데이터를
		올바른 인코딩 타입으로 해석하기 위해서는 sever.xml의 설정을 변경해야 한다.
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// ContextPath : 우리 프로젝트의 최상위 URL. add and remove를 할때 server.xml에 자동으로 추가된다.
